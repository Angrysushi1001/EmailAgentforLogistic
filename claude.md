...
- **Sheet Naming:** `Logistics_Quotes_[domain]` where domain = first part of the **requester's** email (e.g. `raphael.yeong@lxpantos.com` → `lxpantos`). Never use CC'd addresses.
- **Storage:** One Google Sheet per client domain; append new quotes as rows. Multiple cargo items for same client go in the same sheet, distinguished by the Item column.